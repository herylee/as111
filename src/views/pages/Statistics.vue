<template>
  <div>
    <el-form ref="form" :model="form" label-width="80px">
      <el-form-item label="采集Url">
        <el-select v-model="form.selurlclass" placeholder="请选择" @change="selurlclassChange">
          <el-option label="全部" value=""></el-option>
          <el-option v-for="item in option1" :key="item.name" :label="item.name" :value="item.name">
          </el-option>
        </el-select>
        <el-select v-model="form.selurl" placeholder="请选择">
          <el-option label="全部" value=""></el-option>
          <el-option v-for="item in option2" :key="item.name" :label="item.name" :value="item.name">
          </el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="查询日期">
        <el-date-picker v-model="startDate" @change="setStartDate" type="date" placeholder="选择日期"  :picker-options="pickerOptions0">
        </el-date-picker>&nbsp至&nbsp
        <el-date-picker v-model="endDate" @change="setEndDate" type="date" placeholder="选择日期" :picker-options="pickerOptions0">
        </el-date-picker>
      </el-form-item>
      <el-form-item>
        <el-button icon="search" type="primary" @click="onSubmit">查询</el-button>
        <el-button type="primary" @click="downExcel"><i class="fa fa-cloud-download"></i>&nbsp<span>导出</span></el-button>
      </el-form-item>
    </el-form>
    <el-table v-loading="loading" :data="tableData.entities" border style="width: 100%">
      <el-table-column prop="HANDLE" label="环节">
      </el-table-column>
      <el-table-column prop="IP" label="IP地址">
      </el-table-column>
      <el-table-column prop="COMNAME" label="单位">
      </el-table-column>
      <el-table-column prop="ALLALERT" label="报警次数">
      </el-table-column>
      <el-table-column prop="ALLCOUNT" label="操作次数">
      </el-table-column>
      <el-table-column prop="WEEKCOUNT" label="报警比例" :formatter="alertRate">
      </el-table-column>
    </el-table>
    <div class="block" v-show="tableData.total > 0">
      <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="currentPage" :page-sizes="[10, 20, 30, 40]" :page-size="size" layout="total, sizes, prev, pager, next, jumper" :total="tableData.total">
      </el-pagination>
    </div>
  </div>
</template>
<script>
export default {
  created() {
    this.getSelectOption();
  },
  data() {
    return {
      option1: [],
      option2: [],
      start: '',
      end: '',
      form: {
        selurlclass: '',
        selurl: '',
        startDate: '',
        endDate: '',
      },
      pickerOptions0: {
        disabledDate(time) {
          return time.getTime() > Date.now();
        }
      },
      tableData: {},
      loading: false,
      total: 1,
      currentPage: 1,
      size: 10,
      startDate: '',
      endDate: '',
    }
  },
  methods: {
    getSelectOption() {
      this.$axios({
          method: 'get',
          url: '/api/service/as/getUrlSelect',
        }).then((response) => {
          this.option1 = response.data;
        })
        .catch(function(error) {
          this.loading = false;
          console.log(error);
        });
    },
    onSubmit() {
      this.loading = true;
      var data = this.$qs.stringify(this.form);
      // var data = this.form;

      console.log(this.form);
      let url = '/api/service/as/getStatisticsTable?size=' + this.size + '&pageIndex=' + this.currentPage;
      this.$axios({
          method: 'post',
          url: url,
          data: data,
          /***如果qs没有stringify,将下面方法打开才能提交form **/
          // transformRequest: [function(data) {
          //   // Do whatever you want to transform the data
          //   let ret = ''
          //   for (let it in data) {
          //     ret += encodeURIComponent(it) + '=' + encodeURIComponent(data[it]) + '&'
          //   }
          //   return ret
          // }],
          // headers: {
          //   'Content-Type': 'application/x-www-form-urlencoded'
          // }
        }).then((response) => {
          this.tableData = response.data;
          this.loading = false;
        })
        .catch(function(error) {
          this.loading = false;
          console.log(error);
        });

    },
    downExcel() {
      var data = this.$qs.stringify(this.form);
      let url = '/api/service/as/downStatisticsTable?' + data;
      window.open(url);
    },
    //联动菜单
    selurlclassChange(val) {
      var item = this.option1;
      for (var i = 0; i < item.length; i++) {
        var option1 = item[i];
        if (option1.name == val) {
          this.option2 = option1.children;
        }
      }
    },
    alertRate(row, column) {
      return ((row.ALLALERT / row.ALLCOUNT) * 100).toFixed(2) + '%';
    },
    handleSizeChange(val) {
      this.size = val;
      this.onSubmit();
    },
    handleCurrentChange(val) {
      this.currentPage = val;
      this.onSubmit();
    },
    setStartDate(val){
    	this.form.startDate = val;
    },    
    setEndDate(val){
    	this.form.endDate = val;
    }

  },
  watch: {


  }

}

</script>
<style scoped>
.el-input {
  width: 205px;
}

</style>
